...
export async function createProduct(req: Request, res: Response) {
  ...
  try {
    if (!isStructureProduct(datos)) {
      ...
      res.send({
        error:
          "algún dato no es válido, los datos deben ser :: {slug,name,description,price,stock,state,id_brand,id_Product}",
      });
    } else {
...
export async function paginateProducts(req: Request, res: Response) {
  ...
  if (
    ...
    res.send({
      error:
        'los datos no son válidos, se esperaba en body { current : "" , items : "" , options : [{column: "name-column", order: "asc o desc"}] } ',
    });
  }
}
